...
function removeFirstOccurrences(str, value) {
  ...
  if (valueIndex < 0) return str;
  return concatenateStrings(
    str.substring(0, valueIndex),
    str.substring(valueIndex + value.length)
  );
}
...
function removeLastOccurrences(str, value) {
  ...
  if (lastValueIndex < 0) return str;
  return concatenateStrings(
    str.substring(0, lastValueIndex),
    str.substring(lastValueIndex + value.length)
  );
}
...
/**
 * Returns a string in reverse order.s
 *
 * @param {string} str - The input string.
...
function extractNameFromTemplate(value) {
  return value.replace(/Hello, (\w+\s*\w+)!/, '$1');
}
...
function encodeToRot13(str) {
  ...
  const abcdUpper = abcd.toUpperCase();
  return [...str]
    .map((char) => {
      if (abcd.includes(char)) {
        return abcd[(abcd.indexOf(char) + 13) % 26];
      }
      if (abcdUpper.includes(char)) {
        return abcdUpper[(abcdUpper.indexOf(char) + 13) % 26];
      }
      return char;
    })
    .join('');
...
function getCardId(value) {
  let cardIconId = value.split(/\d*\w*/)[1];
  let cardNumber = value.split(cardIconId)[0];

  if (cardNumber === 'A') cardNumber = 1;
  else if (cardNumber === 'J') cardNumber = 11;
  else if (cardNumber === 'Q') cardNumber = 12;
  else if (cardNumber === 'K') cardNumber = 13;

  if (cardIconId === '♣') cardIconId = 0;
  else if (cardIconId === '♦') cardIconId = 1;
  else if (cardIconId === '♥') cardIconId = 2;
  else if (cardIconId === '♠') cardIconId = 3;

  return +cardNumber + 13 * cardIconId - 1;
}
